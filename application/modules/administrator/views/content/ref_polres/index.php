  ...
  <div class='box-header with-border'>
    <h3 class='box-title'>Modul <?=ucfirst($layout_title)?></h3>
      <div class='box-tools pull-right'>
        <a href='<?=site_url(config_item("cpanel")."ref_polres/tambah")?>' id='tambah' class='btn btn-success btn-sm'> Tambah</a>
      </div>
  </div>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                var t = $("#mytable").dataTable({
                    ...
                    processing: true,
                    serverSide: true,
                    ajax: {"url": '<?=base_url(config_item("cpanel")."ref_polres/json")?>', "type": "POST"},
                    columns: [
                        {
                            "data": "id_pol",
                            "orderable": false
           ...
           //   MODAL DETAIL
             $(document).on('click', '#detail', function(e){
                 ...
                 $('.modal-dialog').removeClass('modal-sm');
                 $('.modal-dialog').addClass('modal-md');
                 $('#ModalHeader').html('Detail Polres/Polsek');
                 $('#ModalContent').load($(this).attr('href'));
                 $('#ModalGue').modal('show');
               });
